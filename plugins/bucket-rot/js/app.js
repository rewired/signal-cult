import {BucketRenderer} from './renderer.js';
import {fields,defaults,presets,parsePreset,nextPreset,previewSize,outputTap} from './params.js';
...
let params={...defaults},mode='step-right',presetId='counterflow',presetName='Counterflow';
...
function custom(){presetId='custom';presetName='Untitled';$('preset-select').value='custom';$('preset-status').textContent='Current settings';$('preset-description').textContent='Custom delay and modulation routes.';}
function present(){if(renderer){renderer.params=params;renderer.mode=mode;renderer.present(params.amount,bypass);syncMatrix();}}
...
function apply(preset,id='custom'){
 ...
 $('preset-description').textContent=preset.description||'Imported delay and modulation routes.';
...
for(const f of fields){
 ...
 const update=value=>{if(!Number.isFinite(value))return;params[f.key]=Math.min(f.max,Math.max(f.min,value));range.value=number.value=params[f.key];custom();if(f.key==='memoryScale'){renderer?.setMemoryScale(params.memoryScale);clearMemory();}present();};
...
on($('save-preset'),'click',()=>{
 const preset=parsePreset(JSON.stringify({format:'bucket-rot-preset',version:1,name:presetName,mode,params}));
 const url=URL.createObjectURL(new Blob([JSON.stringify(preset,null,2)+'\n'],{type:'application/json'}));
 const a=document.createElement('a');a.href=url;a.download=(presetName.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'bucket-rot')+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);$('preset-status').textContent='Saved: '+presetName;
});

const cells=[];
for(let i=0;i<64;i++){
 const button=document.createElement('button');button.type='button';button.className='bucket';button.textContent=String(i+1).padStart(2,'0');button.setAttribute('aria-label','View stage '+(i+1));
 on(button,'click',()=>{params.tap=i+1;custom();present();});$('matrix').append(button);cells.push(button);
}
function syncMatrix(){
 const chain=renderer?.chain;const levels=chain?.levels(params);const tap=outputTap(params);
 for(let i=0;i<64;i++){const cell=cells[i];if(!cell)continue;const carrier=levels?.carrier[i]??0,wave=levels?.wave[i]??0;
  cell.style.setProperty('--carrier',carrier);cell.style.setProperty('--wave',wave);
  cell.classList.toggle('filled',Boolean(chain?.slots[i].filled));cell.classList.toggle('trigger',chain?.cursor===i);cell.classList.toggle('selected',tap===i+1);cell.setAttribute('aria-pressed',String(tap===i+1));cell.disabled=Boolean(params.finalOnly);
  cell.title='Stage '+(i+1)+(chain?.slots[i].filled?' · stored image':' · empty')+' · carried '+Math.round(carrier*100)+'% · wave '+Math.round(wave*100)+'%';
 }
 $('view-label').textContent=bypass?'INPUT':params.finalOnly&&!chain?.slots[tap-1].filled?'FILLING FINAL STAGE':'OUTPUT';
 const transfers=params.signalDirection===1?tap-1:64-tap;
...
on($('step-clock'),'click',()=>{if(!renderer||playing)return;renderer.upload(source,params,mode);renderer.tick();present();});
...
function loop(now){
 ...
 try{
  ...
  if(needsFrame&&!document.hidden&&!(source instanceof HTMLVideoElement&&source.seeking)){
   ...
   renderer.update(source,params,mode,delta>0?delta:1/30);renderer.present(params.amount,bypass);syncMatrix();needsFrame=false;lastMediaTime=sourceTime;statsFrames++;
...
function start(){
 if(started)return;
 try{renderer=new BucketRenderer($('preview'));renderer.resize(960,540);started=true;playbackUI();lastRAF=0;statsTime=performance.now();raf=requestAnimationFrame(loop);}catch(error){showError(error.message);}
}
...
const key='bucket-rot.photosensitivity-warning.dismissed';let accepted=false;try{accepted=localStorage.getItem(key)==='true';}catch{}
...
sync();syncMatrix();$('preset-description').textContent=presets[0].description;
