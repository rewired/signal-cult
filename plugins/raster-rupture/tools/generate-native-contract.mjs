...
const styles=['transmission','xerox','dry-ink','thermal','motion'],lines=[];
...
lines.push('struct FactoryPreset { const char* name; int style; std::array<float,static_cast<std::size_t>(ParameterId::Count)> values; std::array<float,3> ink,accent; std::array<std::array<float,2>,9> routes; };',`inline constexpr std::array<FactoryPreset,${presets.length}> kFactoryPresets{{`);
for(const p of presets)lines.push(` {${q(p.name)},${styles.indexOf(p.style)},{{${fields.map(x=>f(p.params[x.key])).join(',')}}},{{${hex(p.colors.ink)}}},{{${hex(p.colors.accent)}}},{{${p.routes.map(r=>`{{${f(r[0])},${f(r[1])}}}`).join(',')}}}},`);
