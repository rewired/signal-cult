...
struct Look { ParameterValues parameters{}; std::array<float,3> ink{{.91f,.89f,.85f}},accent{{1,.15f,.22f}}; std::array<std::array<float,2>,9> routes{}; int style=0,mask_channel=0; bool invert_mask=false,source_as_mask=false; };
struct RenderRequest { ConstImageView source,previous,mask; ImageView output; Look look; double time_seconds=0; };
