...
inline bool nativeSupports(const ParameterValues& values) {
  ...
  if (get(ParameterId::FeedbackAmount) > 0.000001
  ...
  for (int slot = 0; slot < 4; ++slot) {
    ...
    // Feedback displacement/phase routes are harmless while feedback is off. Reject only
    // routes that can activate a native-unsupported processing stage by themselves.
    if (destinationValue == 5 || destinationValue == 16) return false;
  }
  return true;
}
