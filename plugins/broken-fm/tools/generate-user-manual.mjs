...
const workflows = [
  ...
  ["Choose the source", "After the safety warning is acknowledged, Test pattern is active and highlighted by default. Load a video or still image, or select Camera for a live source. Loading a video enables Play; still images and camera feeds run from the application clock. Selecting Test pattern pauses a loaded video or releases the camera."],
...
<section><h2>Header and control behavior</h2><div class="prose"><p><strong>Load media</strong> opens a local video or still image. Camera requests a live source; Test pattern releases an active camera or pauses loaded video. Play resumes video and makes it active. Save frame exports the processed output as a lossless PNG. The preset controls step through 56 factory presets or load/save schema-16 JSON. OSC contains carrier/LFO/matrix controls. Setup contains session assets, Seed, and signal routing. Debug View is an optional lower-right overlay.</p><p>Normal dragging is fine-grained. Hold <strong>Ctrl while dragging</strong> for meaningful coarse snapping. Gray controls are inactive in the current mode. A matrix-modulated control shows <strong>base -&gt; effective</strong> when its effective value differs.</p></div></section>
