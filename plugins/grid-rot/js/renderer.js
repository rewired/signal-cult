import {gridFor,activeArea} from './params.js';
...
uniform sampler2D image;uniform vec2 grid,origin,area,size;
uniform float amount,shift,split,crush,tick,overlay;
float noise(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}
void main(){
 vec4 fresh=texture(image,uv);vec2 cell=floor(vec2(uv.x,1.-uv.y)*grid);
 vec2 local=mod(cell-origin+grid,grid);bool affected=local.x<area.x&&local.y<area.y;
 color=fresh;
 if(affected&&amount>0.){
  ...
  color=vec4(mix(fresh.rgb,damaged,amount),fresh.a);
 }
...
export class GridRenderer {
 ...
 reset(){this.offset=-this.time*(this.params?.rate||0);}
 ...
 state(){const grid=gridFor(this.sourceWidth,this.sourceHeight,this.params.density);return {grid,area:activeArea(grid,this.params,this.mode,this.time,this.offset)};}
 present(amount,bypass=false){if(!this.valid)return;const {grid,area}=this.state();this.draw(this.display,null,{image:this.upload},{grid:[grid.columns,grid.rows],origin:[area.x,area.y],area:[area.width,area.height],size:[this.width,this.height],amount:bypass?0:amount,shift:this.params.shift,split:this.params.split,crush:this.params.crush,tick:area.tick,overlay:!bypass&&amount>0&&this.overlay?1:0});}
